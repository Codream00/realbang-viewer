...
model_path = "models/dohyun_updown_30000/point_cloud/iteration_30000/point_cloud.ply"
camera_path = "models/dohyun_updown_30000/cameras.json"
...
async def create_offer(offer: Offer, session_id: str = Query(...)):
    ...
    def on_datachannel(channel):
        ...
        def on_message(message):
            ...
            if payload["type"] == "camera_update":
                ...
                logging.info(f"Before Euler ${rotation}")
                rotation = Rotation.from_euler("xyz", rotation, degrees=True).as_matrix()
                logging.info(f"After Euler ${rotation}")
    ...
    rotation = Rotation.from_matrix(track.session.renderer.camera.cam_info['rotation']).as_euler('xyz',degrees=True)
    position = track.session.renderer.camera.cam_info['position']

    return {"sdp": pc.localDescription.sdp, "type": pc.localDescription.type, "cam_info": {"position": position,"rotation":rotation.tolist()}}
