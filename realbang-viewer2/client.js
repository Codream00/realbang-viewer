...
let xDeg = 0;
let yDeg = 0;
let offset = new THREE.Vector3(0,0.5,-5)
let radius = 3;
...
const camInfoElement = document.querySelector(".camera-info")
...
async function createPeerConnection() {
    ...
    document.querySelector(".camera-info").innerHTML = 
    `
        <p>position: ${answer.cam_info.position}</p>
        <p>rotation: ${answer.cam_info.rotation}</p>
    `
...
function sendCameraInfoToBackend(position, rotation) {
    ...
    camInfoUpdate(position, rotation);
    dc.send(JSON.stringify(payload));
}

function camInfoUpdate(position, rotation){
    document.querySelector(".camera-info").innerHTML = 
    `
